...
pub fn main() {
   ...
   let user_usdc_second_token_account = keypair(12);
   ...
   let new_farm = Farm {
    ...
    fee_owner: user_usdc_second_token_account.pubkey(),
   ...
   .add_account_with_tokens(user_usdc_second_token_account.pubkey(), mint_address, creator.pubkey(), 0)
   .build();
   ...
   let user_usdc_second_token_account_info = env.get_account(user_usdc_second_token_account.pubkey()).unwrap();
   let initial_user_usdc_second_account_info = TokenAccount::unpack_from_slice(&user_usdc_second_token_account_info.data).unwrap();



   println!(" Initial User USDC Token Account Balance: {}", initial_user_usdc_account_info.amount);
   println!(" Initial User USDC Second Token Account Balance: {}", initial_user_usdc_second_account_info.amount);



   /*creating our pay farm fee instruction, the fee owner account is a second token account owned by the creator
   */
   let pay_farm_fee_instruction = ix_pay_create_fee(
       ...
       &user_usdc_second_token_account.pubkey(),
       &spl_token::ID,
       &farm_program,
       FARM_FEE
   );
   ...
   //vertify that the creators second token account now has the Farm Fee and that the Farm is now enabled 
   let user_usdc_token_account_info = env.get_account(user_usdc_token_account.pubkey()).unwrap();
   let final_user_usdc_account_info = TokenAccount::unpack_from_slice(&user_usdc_token_account_info.data).unwrap();

   let user_usdc_second_token_account_info = env.get_account(user_usdc_second_token_account.pubkey()).unwrap();
   let final_user_usdc_second_account_info = TokenAccount::unpack_from_slice(&user_usdc_second_token_account_info.data).unwrap();
   ...
   println!("Final User USDC Second Token Account Balance: {}", final_user_usdc_second_account_info.amount);

   if initial_user_usdc_account_info.amount == final_user_usdc_second_account_info.amount && final_farm_data.is_allowed == 1 && final_user_usdc_account_info.amount == 0 {
